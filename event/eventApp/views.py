...
class EventAPIView(APIView):
    ...
    def put(self, request, event_id):
        ...
        guests = RSVP.objects.filter(event=event)
                ...
                if serializer.is_valid():
                    ...
                    for guest in guests:
                        send_update.delay(
                            ...
                            f'The Event has been updated. Please check the event details.',
...
class RSVPView(APIView):
    ...
    def post(self, request, event_id):
        ...
        user_event, created = RSVP.objects.get_or_create(
...
class FeedbackView(APIView):
    ...
    def get(self, request, event_id):
        ...
        return Response({'event': event.name, 'feedback': serializer.data}, status=status.HTTP_200_OK)
...
class EventContentView(APIView):
    ...
    def put(self, request, event_id, content_id):
        ...
        if request.user == event.organizer:
            ...
            serializer = EventcontentSerializer(
                event_content, data=request.data, partial=True)
            if serializer.is_valid():
                serializer.save(event=event)
                for guest in guests:
                    send_update.delay(
                        f'Event Update : {event.name}',
                        f'The Event content has been updated. Please check the event details.',
                        settings.EMAIL_HOST_USER,
                        [guest.user.email],
                    )
                return Response(serializer.data, status=status.HTTP_201_CREATED)
